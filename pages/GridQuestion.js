...
import {
  checkOptionFieldsCharacterLimit,
  checkQuestionFieldsCharacterLimit,
  resetQuestion,
  validateCommonUIElements
} from "./AddSurveyQuestionCommonUtils";
...
export async function createGridQuestion(page) {
  ...
  await setViewPort(
    page,
    "div[class='MuiStack-root css-3j596c'] div[class='MuiStack-root css-o0o90z']"
  );
  await expect(page.locator(".css-3j596c")).toHaveScreenshot(
    "Grid_question.png",
    { fullPage: true, maxDiffPixelRatio: 0.05 }
  );
  await resetViewPort(page);
  await validateUIElements(page);
  ...
  await expect(page.locator('button[name="addButton"]').nth(1)).toBeVisible();
  await expect(
  ...
  ).toHaveValue("Grid Question");
  await expect(page.getByPlaceholder("Enter row header")).toHaveValue(
    "Row Header"
  );
...
async function validateUIElements(page) {
  ...
  await expect(page.locator("(//input[@type='checkbox'])[4]")).toBeChecked();
}

async function addColumnDataAndValidateUI(page) {
